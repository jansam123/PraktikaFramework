...
class Plot:
    ...
    def fit(self, model, guess, exclude=None):
        if exclude is not None:
            fit = q.fit(self.xdata.delete(exclude), self.ydata.delete(
                exclude), model, parguess=guess, degree=self.degree)
        else:
            fit = q.fit(self.xdata, self.ydata, model,
                        parguess=guess, degree=self.degree)
    ...
    def plot_fit(self, model, npoints=None, fmt='-', show_dispersion=True):
        ...
        x_fit = np.linspace(self.xdata.values.min(),
                            self.xdata.values.max(), npoints)
        model_vals = model(x_fit, *self.params)
        if self.no_points:
            label = self.leg_label
        else:
            label = None
        self.ax.plot(x_fit, model_vals, fmt, zorder=10,
                     color=self.color, label=label)
        if show_dispersion:
            model_valsPlus = model(
                x_fit, *[param + err for param, err in zip(self.params, self.params_err)])
            model_valsMinus = model(
                x_fit, *[param - err for param, err in zip(self.params, self.params_err)])
            self.ax.fill_between(x_fit, model_valsMinus,
                                 model_valsPlus, color=self.color, alpha=0.2)
