...
class Table(object):
    ...
    def add_all_col(self, index, inline_error, ignore_error):
        for col_id, col in enumerate(self.columns):
            ...
            col_name = self.add_col(
                col, apply_err=apply_err, inline_err=inline_error)
    ...
    def save(self, save_as, file_name):
        if save_as == 'csv':
            self.df.to_csv(file_name + '.csv', sep=";",
                           decimal=",", index=True)

        elif save_as == 'tex':
            if isinstance(self.index, pd.Series):
                table = LatexTable(self.df, True, label=file_name.split(
                    '/')[-1], caption=self.caption)
                table(file_name)
            else:
                table = LatexTable(self.df, label=file_name.split(
                    '/')[-1], caption=self.caption)
                table(file_name)

        elif save_as == 'latex':
            self.df.to_latex(file_name + '.tex', escape=False,
